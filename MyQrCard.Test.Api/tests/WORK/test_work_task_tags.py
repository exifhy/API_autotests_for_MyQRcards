...
class TestWorkTaskTags(BaseTest):

    # @allure.title('Test add tags to task.')
    # @allure.testcase("https://dev.azure.com/melston/HubEx/_workitems/edit/25108")
    # @pytest.mark.skip(reason="Тест на добавления навыка к заявке проходит в - test_delete_tags_from_task")
    # @pytest.mark.regress
    # @pytest.mark.test_case_id(25108)
    # def test_post_add_tags_to_task(self):
    ...
    @allure.title('Test delete tags from task.')
    @allure.testcase("https://dev.azure.com/melston/HubEx/_workitems/edit/25158")
    @pytest.mark.regress
    @pytest.mark.test_case_id(25158)
    def test_delete_tags_from_task(self):
        created_location_id = self.api_es_locations.post_add_location()
        company_id = self.api_es_companies.post_add_our_company()
        self.api_es_company_locations.post_add_company_locations(
            company_id=company_id,
        ...
        tag = self.api_work_task_tags.post_add_tags_to_task(model_task.id)
        self.api_work_task_tags.delete_tags_from_task(model_task.id, tag)
        self.api_work_tasks.delete_task_by_id(model_task.id)
        self.api_es_assets.delete_object_by_id(object_model.id)
        self.api_es_companies.delete_company_by_id(company_id)
        self.api_es_locations.delete_location_by_id(created_location_id)
