...
class TestWorkTaskContacts(BaseTest):

    @allure.title('Test adds contact persons to the task.')
    @allure.testcase("https://dev.azure.com/melston/HubEx/_workitems/edit/24390")
    @pytest.mark.skip(reason="Тест на добавление контакта проходит в - test_delete_contacts_from_task")
    @pytest.mark.regress
    @pytest.mark.test_case_id(24390)
    def test_post_add_contacts_to_task(self):
    ...
    @pytest.mark.regress
    @pytest.mark.test_case_id(24391)
    def test_delete_contacts_from_task(self):
        created_location_id = self.api_es_locations.post_add_location()
        company_id = self.api_es_companies.post_add_our_company()
        location_id = self.api_es_locations.post_add_location()
        contact_id = self.api_common_contacts.post_add_contacts()
        ...
        try:
            ...
            self.api_work_task_contacts.delete_contacts_from_task(
                model_task.id,
                contact_id.contact[0]
            )
        finally:
