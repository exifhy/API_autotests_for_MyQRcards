...
@allure.title("Add photo gallery -> verify -> delete")
@pytest.mark.e2e
@pytest.mark.employee
class TestEmployeeInvitationAddPhotoDeleteFlow:
    @allure.title("Employee created and 5 attachments uploaded")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_01_employee_created(self, employee_addphoto_flow):
        assert employee_addphoto_flow["account_id"] > 0
        assert employee_addphoto_flow["invite_id"] > 0

        attachment_ids = employee_addphoto_flow["attachment_ids"]
        assert isinstance(attachment_ids, list)
        assert len(attachment_ids) == 5

        allure.attach(
            str(employee_addphoto_flow["invite_id"]),
            "invite_id",
            allure.attachment_type.TEXT,
        )
        allure.attach(
            str(employee_addphoto_flow["account_id"]),
            "account_id",
            allure.attachment_type.TEXT,
        )
        allure.attach(
            "\n".join(map(str, attachment_ids)),
            "attachment_ids",
            allure.attachment_type.TEXT,
        )

    @allure.title("PUT photo attribute")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_02_add_photo_attributes(self, lk_api, employee_addphoto_flow):
        account_id = int(employee_addphoto_flow["account_id"])
        card_id = int(employee_addphoto_flow["card_id"])
        attachment_ids = employee_addphoto_flow["attachment_ids"]
    ...
    @allure.title("Photo visible via 085.2")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_03_photo_visible_in_0852(self, lk_api, employee_addphoto_flow):
        account_id = int(employee_addphoto_flow["account_id"])
        card_id = int(employee_addphoto_flow["card_id"])
        attachment_ids = employee_addphoto_flow["attachment_ids"]
        ...
        assert ok, (
            ...
            "with uploaded values not visible in 085.2"
        )

    @allure.title("Delete invitation")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_04_delete_employee(self, lk_api, employee_addphoto_flow):
        sub_id = int(employee_addphoto_flow["subscription_id"])
        invite_id = int(employee_addphoto_flow["invite_id"])
        ...
        employee_addphoto_flow["deleted"] = True
