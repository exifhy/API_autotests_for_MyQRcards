...
from src.constants.attributes import AVATAR_ATTRIBUTE_ID_V2
...
@allure.title("Create card -> Upload avatar gallery -> Delete")
@pytest.mark.mobile
@pytest.mark.e2e
class TestMobileAppCardUploadAvatar:
    @allure.title("Create mobile card")
    def test_01_create_card(self, mobile_api, mobile_avatar_gallery_flow):
        response = mobile_api.post("/cards/V2.0", json=mobile_avatar_gallery_flow["payload"])
        ...
        mobile_avatar_gallery_flow["card_id"] = int(card_id)

    @allure.title("Get card after create")
    def test_02_get_card_after_create(self, mobile_api, mobile_avatar_gallery_flow):
        card_id = mobile_avatar_gallery_flow["card_id"]
    ...
    @allure.title("Upload avatar photos")
    def test_03_upload_avatar_photos(self, mobile_api, mobile_avatar_gallery_flow):
        files = list_image_files(mobile_avatar_gallery_flow["media"]["photos_dir"])
        uploaded_ids = []
        ...
        mobile_avatar_gallery_flow["uploaded_ids"] = uploaded_ids

    @allure.title("Update card with uploaded gallery")
    def test_04_update_card_set_gallery(self, mobile_api, mobile_avatar_gallery_flow):
        card_id = mobile_avatar_gallery_flow["card_id"]
        uploaded_ids = mobile_avatar_gallery_flow["uploaded_ids"]
        assert card_id, "card_id empty (test_01 failed?)"
    ...
    def test_05_get_card_contains_gallery_ids(self, mobile_api, mobile_avatar_gallery_flow):
        card_id = mobile_avatar_gallery_flow["card_id"]
        uploaded_ids = mobile_avatar_gallery_flow["uploaded_ids"]
        assert card_id, "card_id empty (test_01 failed?)"
        ...
        assert card_json is not None, "Card gallery does not contain all uploaded attachment ids"

    @allure.title("Delete mobile card")
    def test_06_delete_card(self, mobile_api, mobile_avatar_gallery_flow):
        card_id = mobile_avatar_gallery_flow["card_id"]
        assert card_id, "card_id empty (test_01 failed?)"
        ...
        mobile_avatar_gallery_flow["deleted"] = True
