...
class TestAuthzServiceTokens(BaseTest):
    ...
    @pytest.mark.regress
    @pytest.mark.test_case_id(23488)
    ...
    def test_post_user_api_token_generation(self):
        ...
        model_member = self.api_adm_tenant_members.get_returns_api_user_in_current_tenant(
            access_token=model_basic_token.access_token
        )
        self.api_adm_tenant_members.delete_tenant_member_by_id(
            tenant_member_id=model_member.id
        )
    ...
    @pytest.mark.regress
    @pytest.mark.test_case_id(23489)
    @pytest.mark.skip(reason="The user api token is used for the autotest project. Updating API user token is denied.")
