...
class AdmRolePermissionsExtAPI(Helper):
    ...
    def post_role_permissions_ext(self, role_id: int, *permissions_ids: int | tuple):
        start = time.time()
        ...
        logger.success(f'Successfully add role permissions Ext.')
        return model

    @allure.step("Add all task role permissions Ext.")
    def post_role_permissions_ext_all_task(self, role_id: int):
        start = time.time()
        response = requests.post(
            url=self.endpoints.post_role_permissions_ext_endpoint,
            headers=self.headers.basic_header(get_token()),
            json=self.payloads.post_role_permissions_ext_all_task_payload(
                role_id
            )
        )
        end = time.time()
        ...
        assert response.status_code == HTTPStatus.CREATED, \
            (f'Expected status code {HTTPStatus.CREATED}, '
             f'but got {response.status_code}, {data_response}')
        model = RolePermissionsExtListResponseModel(results=response.json())
        logger.success(f'Successfully add all task role permissions Ext.')
        return model

    @allure.step("Delete role permissions Ext.")
    def delete_role_permissions_ext(self, role_id: int, *permissions_ids: int | tuple):
        start = time.time()
        ...
        assert response.status_code == HTTPStatus.ACCEPTED, \
            (f'Expected status code {HTTPStatus.ACCEPTED}, '
             f'but got {response.status_code}, {data_response}')
        logger.success(f'Successfully delete role ID {role_id} permissions Ext ID {permissions_ids}.')
        return None
