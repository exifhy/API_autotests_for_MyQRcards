...
class ExportUsersAPI(Helper):
    ...
    def get_export_list_customers_by_user_id(
        ...
        assert sheet['A3'].value == 'Фамилия*', f'Expected Фамилия*, but got {sheet['B3'].value}'
        assert sheet['A4'].value == surname.strip(), f'Expected {surname.strip()}, but got {sheet['B4'].value}'
        assert sheet['B3'].value == 'Имя*', f'Expected Имя*, but got {sheet['C3'].value}'
        assert sheet['B4'].value == name.strip(), f'Expected {name.strip()}, but got {sheet['C3'].value}'
        assert sheet['C3'].value == 'Отчество', f'Expected Отчество, but got {sheet['D3'].value}'
        assert sheet['D3'].value == 'Пол', f'Expected Пол, but got {sheet['F3'].value}'
        assert sheet['E3'].value == 'Телефон*', f'Expected Телефон*, but got {sheet['G3'].value}'
        assert f"+{sheet['E4'].value}" == phone.strip(), f'Expected {phone.strip()}, but got +{sheet['G4'].value}'
        assert sheet['F3'].value == 'Электронная почта*', f'Expected Электронная почта*, but got {sheet['H3'].value}'
        assert sheet['F4'].value == email.strip(), f'Expected {email.strip()}, but got {sheet['H4'].value}'
        assert sheet['H3'].value == 'Роль пользователя*', f'Expected Роль пользователя*, but got {sheet['L3'].value}'
        assert sheet['H4'].value == role.strip(), f'Expected {role.strip()}, but got {sheet['L4'].value}'
        assert sheet['I3'].value == 'Участок', f'Expected Участок, but got {sheet['N3'].value}'
        assert sheet['I4'].value == district_name.strip(), f'Expected {district_name.strip()}, but got {sheet['N4'].value}'
        assert sheet['J3'].value == 'Компания', f'Expected Компания, but got {sheet['P3'].value}'
        assert sheet['J4'].value == company_name.strip(), f'Expected {company_name.strip()}, but got {sheet['P4'].value}'
