...
class CommonApplicationsAPI(Helper):
    ...
    @allure.step("Get and return list of applications.")
    def get_and_return_list_applications(self) -> list:
        start = time.time()
        response = requests.get(
            url=self.endpoints.get_list_of_branches_endpoint,
            headers=self.headers.basic_header(get_token())
        ...
        assert response.status_code in {HTTPStatus.OK, HTTPStatus.PARTIAL_CONTENT}, \
            f'{response.status_code}, {response.json()}'
        model = SuccessGetApplicationResultModel(root=response.json())
        list_of_app = list(model.root.keys())
        logger.warning(f'Successfully received list of branches (applications) {list_of_app}.')
        return list_of_app
