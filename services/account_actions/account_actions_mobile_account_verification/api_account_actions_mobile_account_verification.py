...
class AccountActionsMobileAccountVerificationAPI(Helper):
    ...
    @allure.step("POST /accountActions/MobileAccountVerification/silent")
    def create_mobile_account_verification_silent(
        self,
        *,
        push_token: str,
        client_name: str,
        ...
        response = self._call(
            "POST",
            url=self.endpoints.create_mobile_account_verification_silent_endpoint,
            headers=headers,
            json=request_payload,
            auth=HTTPBasicAuth(login, password),
        )
        assert response.status_code in (
            HTTPStatus.OK, HTTPStatus.CREATED, HTTPStatus.ACCEPTED, HTTPStatus.CONFLICT,
        ), f"Unexpected status on MobileAccountVerification/silent: {response.status_code}: {response.text}"
        data = response.json() if response.text else {}
        if isinstance(data, list):
            data = {"errors": data}
        return response, AccountActionCreateModel(**data), request_payload
