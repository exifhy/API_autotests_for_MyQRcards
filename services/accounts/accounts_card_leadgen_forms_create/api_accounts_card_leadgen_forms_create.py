...
from services.accounts.accounts_card_leadgen_forms_create.endpoints import Endpoints
...
class AccountsCardLeadGenFormsCreateAPI(Helper):
    def __init__(self):
        self.endpoints = Endpoints()

    @allure.step("POST /Accounts/{account_id}/Cards/{card_id}/leadGenForms")
    def create_accounts_card_leadgen_form(
        self,
        account_id: int,
        card_id: int,
        *,
        ...
        response = self._call(
            "POST",
            url=self.endpoints.create_accounts_card_leadgen_form_endpoint.format(
